...
const signup = (req, res) => {
  ...
  const { userName, email, password, displayName, address, phoneNumber, logo } =
    req.body;
      ...
      (err, result) => {
        ...
        User.findByEmail(email, (err, existingUser) => {
          if (err) {
            return res
              .status(500)
              .json({ error: "Error finding user after create" });
          }
          res
            .status(201)
            .json({
...
const updatePassword = (req, res) => {
  ...
  User.findByEmail(req.user.email, (err, user) => {
    ...
    User.comparePassword(oldPassword, user.password, (err, isMatch) => {
      ...
      if (isMatch) {
        User.updatePassword(newPassword, req.user.id, (err, result) => {
          if (err) {
...
const updateUser = (req, res) => {
  ...
  const { userId, userName, email, displayName, address, phoneNumber } =
    req.body;
